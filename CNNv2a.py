...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 从文件名中提取标签（假设文件名为"3_abc.jpg"，则标签为3）
...
def deal_data(data_dir, batch_size=1):
    ...
    transform = transforms.Compose([
        # 将numpy数组转换为PIL图像格式
        # 调整图像大小，短边为28像素（保持长宽比）
        # 从中心裁剪为28x28像素（确保尺寸一致）
        # 转换为单通道灰度图（MNIST是灰度数据集）
        # 增强对比度3倍（使数字更清晰）
        # 颜色反转（白底黑字 -> 黑底白字）
        # 转换为Tensor格式并归一化到[0.0, 1.0]
        # 标准化（使用MNIST的均值和标准差）
    ])
...
def main():
    ...
    # 测试图像文件夹路径
    image_path =  os.path.join(script_dir, "data/numbersbyMNIST")
