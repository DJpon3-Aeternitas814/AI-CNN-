...
def deal_data(data_dir, batch_size=1):
    ...
    transform = transforms.Compose([
        # 1. 将numpy数组转换为PIL图像格式（假设输入为numpy数组）

        # 2. 调整图像大小（短边缩放到28像素，保持长宽比）

        # 3. 中心裁剪确保尺寸严格为28x28

        # 4. 转换为单通道灰度图（与MNIST格式对齐）

        # 5. 增强对比度（使用ColorJitter增强3倍对比度）

        # 6. 颜色反转（白底黑字 -> 黑底白字，使用PIL的invert操作）

        # 7. 转换为Tensor并自动归一化到[0.0, 1.0]
    ...
    loader = DataLoader(
        ...
        num_workers=2        # 使用2个子进程加载数据（加快数据读取）
    )
...
def main():
    ...
    # 模型文件路径
    model_path = os.path.join(script_dir, "model/mnist_cnn.pth")
    ...
    # 测试图像文件夹路径
    # image_path = "./data/numbersbyMNIST/label"
    image_path =  os.path.join(script_dir, "data/numbersbyHAND")
